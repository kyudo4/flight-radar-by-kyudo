(() => {
  ...
  async function init() {
    ...
    $("loginForm").onsubmit = async e => { e.preventDefault(); const email = $("loginEmail").value.trim(); const { error } = await client.auth.signInWithOtp({ email, options: { emailRedirectTo: location.origin + location.pathname } }); message(error ? error.message : "Link logowania został wysłany na e-mail.", !error); };
  ...
  async function loadApp() {
    ...
    show("authView", false); show("appView"); show("signOutButton"); $("userBadge").textContent = user.email || "zalogowany";
    ...
    $("connectTelegramButton").onclick = connectTelegram;
  ...
  async function connectTelegram() {
    if (!cfg.telegramBotUsername || cfg.telegramBotUsername.includes("YOUR_")) { alert("Administrator nie skonfigurował jeszcze nazwy bota Friends."); return; }
    const raw = crypto.randomUUID().replaceAll("-", ""); const hash = await hashToken(raw); const { error } = await client.from("telegram_link_tokens").insert({ token_hash: hash, user_id: user.id });
    if (error) { alert(error.message); return; }
    const link = `https://t.me/${cfg.telegramBotUsername.replace(/^@/, "")}?start=${raw}`;
    window.open(link, "_blank", "noopener"); alert("Otworzyłem Telegram. Naciśnij Start w bocie — połączenie zostanie wykryte przy najbliższym skanie.");
  }

  async function loadAdmin() {
    const { data, error } = await client.from("profiles").select("id,email,display_name,status,role,last_seen_at,created_at").order("created_at"); if (error) throw error;
    const users = data || []; $("seatCount").textContent = `${users.filter(x => x.status === "active").length}/10 aktywnych`; $("userList").innerHTML = users.map(u => `<div class="user-row"><span><strong>${esc(u.display_name || u.email)}</strong><br><small class="muted">${esc(u.email)}</small></span><span class="status-${esc(u.status)}">${esc(u.status)}</span>${u.id === user.id ? "<span class='muted'>Ty</span>" : `<button class="secondary" data-user="${u.id}" data-status="${u.status}">${u.status === "active" ? "Zawieś" : "Aktywuj"}</button>`}</div>`).join("");
    document.querySelectorAll("[data-user]").forEach(btn => btn.onclick = async () => { const next = btn.dataset.status === "active" ? "suspended" : "active"; await client.from("profiles").update({ status: next }).eq("id", btn.dataset.user); await loadAdmin(); });
