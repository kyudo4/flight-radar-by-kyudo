...
begin
  ...
  if budget <= 0 or duration <= 0 or duration > 24 or stops < 0 or stops > 9 then
...
create policy monitors_owner_all on public.monitors for all using (public.is_active_user(user_id) and (user_id = auth.uid() or public.is_admin())) with check (public.is_active_user(user_id) and (user_id = auth.uid() or public.is_admin()));
...
create policy matches_owner_read on public.user_matches for select using (public.is_active_user(user_id) and (user_id = auth.uid() or public.is_admin()));
create policy matches_owner_update on public.user_matches for update using (public.is_active_user(user_id) and (user_id = auth.uid() or public.is_admin())) with check (public.is_active_user(user_id) and (user_id = auth.uid() or public.is_admin()));
create policy connections_self_read on public.telegram_connections for select using (public.is_active_user(user_id) and (user_id = auth.uid() or public.is_admin()));
create policy connections_self_delete on public.telegram_connections for delete using (public.is_active_user(user_id) and (user_id = auth.uid() or public.is_admin()));
...
create policy feedback_owner_all on public.feedback for all using (public.is_active_user(user_id) and user_id = auth.uid()) with check (
