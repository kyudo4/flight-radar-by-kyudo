-- An empty max_duration_h means that the complete connection has no time cap.
...
-- Existing monitors used 24 h or lower as a mandatory cap. Remove that old
-- setting so the new default is unlimited for every already-created monitor.
update public.monitors
set filters = filters - 'max_duration_h'
where filters ? 'max_duration_h';
